...
const HotelRow = ({ hotel }) => (
  <a href="{hotel.url}" target="_blank">
    <div className="hotels">
      <div className="hotels_photo"><img src={hotel.thumbUrl} alt={hotel.name} /></div>
      <div className="hotels_name">{hotel.name}</div>
      <div className="hotels_price">{hotel.price ? `${hotel.price.toString().replace(/(\d+?)(?=(?:\d{3})+$)/g, x => x + ',')}円〜` : '空室なし'}
      </div>
      ...
      <div className="hotels_count">{hotel.count}</div>
    </div>
  </a>
);
